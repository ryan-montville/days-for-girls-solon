...
function submitData() {
    clearMessages();
    let mailingFormData: FormData = new FormData(mailingListForm);
    //Create contact entry object
    let newContactEntry: ContactEntry = {
        contactId: 0,
        fullName: "",
        email: "",
        volunteerType: ""
    ...
    if (phoneValue !== null && phoneValue.toString().replace(/\D/g, '') !== '') {
        let parsedPhone: number = parseInt(phoneValue.toString().replace(/\D/g, ''));
        if (isNaN(parsedPhone)) {
            createMessage("Please enter a valid phone number", "main-message", "error");
            return;
        } else {
            let lengthCheck: number = String(Math.abs(parsedPhone)).length
