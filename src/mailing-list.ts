...
const mailingListForm = document.getElementById('mailingList') as HTMLFormElement;
...
function submitData() {
    ...
    if (phoneValue !== null && phoneValue.toString().replace(/\D/g, '') !== '') {
        const parsedPhone: number = parseInt(phoneValue.toString().replace(/\D/g, ''));
    ...
    const formAction: string = "https://docs.google.com/forms/d/e/1FAIpQLSeI082sAdNAfYFnAbLUfuL3dqpz8B7CPKZfxCI_MlT9x-cmAg/formResponse";
    fetch(formAction, {
        method: 'POST',
        body: formData,
        mode: 'no-cors'
        ...
        .then(response => {
            ...
            storeMessage("You have been signed up for our mailing list", "main-message", "check_circle");
            window.location.href = 'index.html';
        })
        .catch(error => {
            //Create an error message
            console.error('Network Error:', error);
            createMessage("Error. Please try reloading the page", "main-message", "error");
        });
}
...
initializeApp('Mailing List', 'Mailing List').then(() => {
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            const fullNameInput = document.getElementById('fullNameInput') as HTMLFormElement;
        fullNameInput.value = user.displayName;
        const emailInput = document.getElementById('emailInput') as HTMLFormElement;
